...
describe("TransactionController_Tests", () => {
  ...
  describe("POST /api/v1/transactions/withdraw", () => {
    ...
    it("Test_Withdraw It should return 400 if the account type is not customer", async () => {
      // 1. Generate and create a valid zone
      // a. generate zone
      const genZone = UnitTest.GenRandomValidZone(adminUser.id);
      // b. create zone
      ...
      // a. generate account
      const genAccount = UnitTest.GenRandomValidCustomerAccount(customer.id);
      genAccount.type = "user"; // set account to user account
      // b. create account
      const account = await Account.create(genAccount);
      // 4. Generate and integer amount between 100 and 400
      const amount = RandomVal.GenRandomSmallAmount();
    ...
    it("Test_Withdraw It should return 400 for insufficient customer funds", async () => {
      // 1. Generate and create a valid zone
      // a. generate zone
      const genZone = UnitTest.GenRandomValidZone(adminUser.id);
      // b. create zone
      ...
      // a. generate account
      const genAccount = UnitTest.GenRandomValidCustomerAccount(customer.id);
      genAccount.balance = 20000; // set balance to 20000
      // b. create account
      const account = await Account.create(genAccount);
      // 4. Generate and integer amount between 100 and 400
      const amount = 25000;
